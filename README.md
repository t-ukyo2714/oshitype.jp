...
- `vercel.json` は最小構成（`framework: nextjs` のみ）にし、Install/Build Command は Vercel デフォルトに任せてください。
- エラー `No Next.js version detected` が出る場合は、ほぼ Root Directory の設定不一致です。
- Project Settings で Build & Development Settings の Install/Build Command を上書きしている場合は、いったん解除してください。
